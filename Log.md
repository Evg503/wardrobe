...
**Коренные причины (3 одновременно):**

1. **Выделение `ByteBuffer.allocateDirect` каждый кадр.** В `drawCameraFrame()` буфер `QUAD_COORDS` аллоцировался заново 60 раз в секунду. В `updateTransformedTexCoords()` — ещё один `allocateDirect` на кадр. 1.2 МБ прямых аллокаций/сек — GC не успевал освобождать, eventual OutOfMemoryError.

2. **`processPlanes()` вызывался каждый кадр.** Каждый фрейм ARCore генерировал обновления плоскостей, которые шли `glSurfaceView.post {}` → очередь GL-потока накапливалась быстрее, чем Flutter успевал забирать события через EventChannel, что давало задержки (фризы) и рост памяти.

3. **Нет защиты от параллельных `post`-задач.** Если Flutter не успевал обработать предыдущий callbcak, новый `glSurfaceView.post` добавлялся в очередь без проверки, усиливая накопление.

**Фикс (`android/app/src/main/kotlin/com/example/wardrobe/ArCoreView.kt`):**

1. **Буферы аллоцируются один раз.** `QUAD_COORDS` и трансформированные UV аллоцированы в `onSurfaceCreated`, переиспользуются каждый кадр через `.position(0) + .put() + .position(0)`.
